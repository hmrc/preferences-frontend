...
import views.html.sa.prefs.cohorts.i_page7
import utils.TestData.EMPTY_STRING

class ReadOnlyIPage7Spec extends PlaySpec with GuiceOneAppPerSuite with LanguageHelper with ConfigHelper {

  override implicit lazy val app: Application = fakeApp
  implicit lazy val hostContext: HostContext = new HostContext(returnUrl = EMPTY_STRING, returnLinkText = EMPTY_STRING)

  val template: i_page7 = app.injector.instanceOf[i_page7]
    ...
    "render the correct content in english" when {
      "form has no error" in {
        ...
        document
          .getElementsByTag("li")
          .get(0)
          .text() mustBe "Legal notice to file tax return"

        document
          .getElementsByTag("li")
          .get(1)
          .text() mustBe "Late filing penalty notice"

        document
          .getElementsByTag("li")
          .get(2)
          .text() mustBe "Late payment penalty notice"
    ...
    "render the correct content in welsh" when {
      "form has no error" in {
        ...
        document
          .getElementsByTag("li")
          .get(0)
          .text() mustBe "Hysbysiad cyfreithiol i gyflwyno Ffurflen Dreth"
        ...
        document
          .getElementsByTag("li")
          .get(2)
          .text() mustBe "Hysbysiad o gosb am dalu’n hwyr"
